...
export function createPaymentInstruction(
  ...
  // pre-lock read into a duplicate or a raw constraint error.
  ...
  brepo.withBankingTx(() => {
    ...
    if (existingByKey) {
      if (matchesRequest(existingByKey)) {
        created = existingByKey;
        return;
      }
    ...
    switch (eventType) {
      ...
      case "payment.failed": {
        ...
        if (applied) {
          ...
          // event (including the transaction transition above) rolls
          // back — the two records can never diverge.
          if (
            !brepo.transitionInstructionGuarded(
              instruction.id,
              ["SUBMITTED_TO_PROVIDER", "PROCESSING"],
      ...
      case "payment.returned": {
        ...
        if (applied) {
          ...
          // the whole event back (no transaction/instruction divergence).
          if (
