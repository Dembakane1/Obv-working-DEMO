...
export function effectiveStatus(permit: Permit, atIso = new Date().toISOString()): PermitStatus {
  if ((permit.status === "ISSUED" || permit.status === "ACTIVE") && permit.expiresAt && permit.expiresAt < atIso) {
    return "EXPIRED";
  }
...
export function createPermit(
  ...
  const status = (input.status?.trim() || "UNKNOWN") as PermitStatus;
  if (!PERMIT_STATUSES.includes(status)) throw new PermitError(`Unknown permit status ${status}`);
  if (
    input.expiresAt && input.issuedAt && input.expiresAt < input.issuedAt && !input.legacyImport
  ) {
  ...
  const permit: Permit = {
    ...
    effectiveAt: input.effectiveAt?.trim() || null,
...
export function updatePermit(
  ...
  if (nextExpires && nextIssued && nextExpires < nextIssued) {
...
export function recordOfficialSource(
  ...
  if (input.inspectionId?.trim()) {
    ...
    if (!insp || insp.projectId !== project.id) throw new PermitError("Inspection not found in this project", 404);
  ...
  if (input.artifactDataUrl?.trim()) {
    ...
    const safeName = (input.artifactFilename ?? "artifact")
      .replace(/[^a-zA-Z0-9._-]+/g, "_")
      .slice(0, 80);
    const dir = path.join(UPLOADS_DIR, "official-sources");
    fs.mkdirSync(dir, { recursive: true });
    sourceDocumentPath = path.join("official-sources", `${sourceArtifactHash.slice(0, 16)}-${safeName}`);
...
export function validatePermitIntegrity(projectId: string): PermitIntegrityFinding[] {
  ...
  for (const i of inspections) {
    ...
    // Mandatory official source for recorded PASSED results.
    const req = repo.getInspectionRequirement(i.milestoneId);
    if (i.result === "PASSED" && req?.officialSourceRequired && repo.listOfficialSourcesForInspection(i.id).length === 0) {
      findings.push({ severity: "CRITICAL", code: "MANDATORY_OFFICIAL_SOURCE_MISSING", detail: `Inspection ${i.id} is PASSED but the configured official source record is missing` });
    }
  }
