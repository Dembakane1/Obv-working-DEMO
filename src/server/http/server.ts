...
async function handle(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  if (
    await handleOfficialSourceRoutes({
      ...
      getUser: () => {
        const u = currentUser(req);
        if (!u) throw new OfficialSourceError("Select a demo user first", 401);
        return u;
      },
      signInLocation: signInPath(),
      navFor,
  ...
  if (method === "GET" && pathname === "/executive") {
    ...
    const evidenceQuality = eviAnalytics
      ...
      : null;
    // Advisory Official Sources summary — same doctrine: derived on read,
    // viewer-scoped, never a control decision.
    const srcAnalytics = officialSourcesSvc.canViewSources(user!)
      ? officialSourcesSvc.officialSourceAnalytics(user!)
      : null;
    const officialSources = srcAnalytics
      ? {
          coveragePct: srcAnalytics.coverage.coveragePct,
          openReviews: srcAnalytics.reviewBacklog.open ?? 0,
          enforcementAlerts: srcAnalytics.enforcementAlerts,
          licenseAlerts: srcAnalytics.licenseAlerts,
...
server.listen(PORT, () => {
  ...
  console.log(officialSourcesSvc.officialSourcesStartupNotice());
  if (demoAuthEnabled()) {
