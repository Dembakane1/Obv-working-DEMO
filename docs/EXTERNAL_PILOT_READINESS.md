...
| Draw documents | **INTENTIONALLY_DISABLED** (bytes) | Draw/inspection document uploads record METADATA + SHA-256 only; file bytes are not stored (`file_path` null by design). For pilot 1 this is the documented custody model — the lender retains originals; OBV records integrity hashes. Revisit (object storage) in a later milestone if custody must move. |
