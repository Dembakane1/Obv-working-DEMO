...
|---|---|---|
| **`rowid` as an ORDER BY tiebreaker** | 8 | PostgreSQL has no `rowid`. Queries like `ORDER BY created_at, rowid` rely on SQLite's implicit insertion order to make same-timestamp rows deterministic. **This is the single most important item in this document**: draw line items, draw events, documents and evidence links all order this way, and a non-deterministic order changes what a lender sees. **Hard precondition of cutover:** every `rowid` tiebreaker must be replaced by an explicit deterministic tie-breaker — an immutable monotonic sequence column (`seq BIGSERIAL`), a created-order ID, or another stable sortable identifier — *before* SQLite → PostgreSQL cutover, and the replacement must reproduce the current ordering on migrated rows. The ordering lenders see must not change after migration. Never fixed after the fact. |
